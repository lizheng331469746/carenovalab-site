...
export default function ContactPage(){return <>
  ...
  <section className="section section-white"><div className="container"><SectionHeading eyebrow="Product Team" title="Choose Willow or Jasmine" text="Both consultants work under the CareNova Lab team structure. Select the person you prefer based on your existing communication or project needs."/><div className="team-grid">{siteConfig.consultants.map((c,i)=><article className="team-card" key={c.id}><div className={`avatar avatar-${i+1}`}>{c.image ? <img src={c.image} alt={`${c.name}, ${c.title}`} /> : c.name[0]}</div><div><h3>{c.name}</h3><p className="consultant-title">{c.title}</p><p>{c.shortBio}</p><DirectConsultantLink consultant={c}/></div></article>)}</div><div className="hero-actions"><a className="button button-outline" href={`mailto:${siteConfig.email}?subject=CareNova%20Lab%20Product%20Inquiry`}>Send an Email Inquiry</a></div></div></section>
